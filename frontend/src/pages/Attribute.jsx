...
const Attribute = () => {
    ...
    const handleAddModel = async (brandId) => {
        handleCreate('models', brandId, 'brandId');
    };

    const handleAddCapacity = async (modelId) => {
        handleCreate('capacities', modelId, 'modelId');
    };

    const handleAddColor = async (modelId) => {
        handleCreate('colors', modelId, 'modelId');
    };

    const handleAddVersion = async (modelId) => {
        handleCreate('versions', modelId, 'modelId');
    };

    const handleAddImageUrl = async (colorId) => {
        console.log("colorId")
        const url = prompt(`请输入新的图片地址：`);
        if (url) {
            axios.put(`${process.env.REACT_APP_API_URL}/colors/${colorId}`, { image: url }).then(response => {
                                    ...
                                    <ul>
                                        ...
                                        <li>颜色：
                                            {model.colors.map(color => (
                                                <span onClick={() => handleAddImageUrl(color.id)}>{color.name}、</span>
                                            ))}
